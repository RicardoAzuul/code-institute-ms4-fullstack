...
def cache_checkout_data(request):
    ...
    try:
        ...
        messages.error(
            request, 'Payment cannot be processed. Try again later.')
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            request.session['save_info'] = 'save-info' in request.POST # noqa
            return redirect(reverse(
                'checkout_success', args=[order.order_number]))
        else:
            messages.error(
                request, 'Error in form. Please double check information.')
